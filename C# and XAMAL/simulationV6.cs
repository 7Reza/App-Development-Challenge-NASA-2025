    ...
    public partial class MainWindow : Window
    {
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            if (rocketPositionData.Count > 0 && moonPositionData.Count > 0 && timeData.Count > 0)
            {
                ...
                currentPositionIndex = (currentPositionIndex + 1) % Math.Min(
                    rocketPositionData.Count,
                    Math.Min(moonPositionData.Count, timeData.Count));
            }
        }
        ...
        private void CompositionTarget_Rendering(object sender, EventArgs e)
        {
            if (isAnimating && rocketPositionData.Count > 0 && moonPositionData.Count > 0 && timeData.Count > 0)
            {
                ...
                // Update the UI with current positions
                Dispatcher.Invoke(() =>
                {
