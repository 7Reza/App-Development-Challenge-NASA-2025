...
namespace SpherePositionTracker
{
    public partial class MainWindow : Window
    {
        ...
        private void InitializeScene()
        {
            ...
            /* CENTRAL REFERENCE SPHERE (EARTH) */
            MeshBuilder staticSphereMeshBuilder = new MeshBuilder();
            staticSphereMeshBuilder.AddSphere(new Point3D(0, 0, 0), 6.738); // Earth radius

            // Create texture material for Earth
            ImageBrush earthTexture = new ImageBrush();
            BitmapImage earthImage = new BitmapImage(new Uri("C:\\Users\\Mohammad Reza\\Projects\\NASA\\C#\\data\\8k_earth_daymap.jpg", UriKind.Absolute));
            // Or use a file path: new BitmapImage(new Uri("C:/path/to/earth.jpg", UriKind.Absolute));
            earthTexture.ImageSource = earthImage;

            GeometryModel3D staticSphereModel = new GeometryModel3D();
            staticSphereModel.Geometry = staticSphereMeshBuilder.ToMesh();
            staticSphereModel.Material = new DiffuseMaterial(earthTexture);
            ...
            /* MOON SPHERE (TEXTURED) */
            MeshBuilder dynamicMoonSphereMeshBuilder = new MeshBuilder();
            dynamicMoonSphereMeshBuilder.AddSphere(new Point3D(-30, 20, 12), 1.737); // Moon radius

            // Create texture material for Moon
            ImageBrush moonTexture = new ImageBrush();
            BitmapImage moonImage = new BitmapImage(new Uri("C:\\Users\\Mohammad Reza\\Projects\\NASA\\C#\\data\\8k_moon.jpg", UriKind.Absolute));
            // Or use a file path: new BitmapImage(new Uri("C:/path/to/moon.jpg", UriKind.Absolute));
            moonTexture.ImageSource = moonImage;

            dynamicMoonSphereModel = new GeometryModel3D();
            dynamicMoonSphereModel.Geometry = dynamicMoonSphereMeshBuilder.ToMesh();
            dynamicMoonSphereModel.Material = new DiffuseMaterial(moonTexture);
